...
if (isset($_POST['submit'])) 
{
	...
	else 
	{
		...
		else
		{
			...
			$conn = db_conn();
			$selectQuery = "SELECT * FROM `deliveryman` WHERE email = :email";
			try
		    {
	...
	else 
	{
		...
		if (preg_match("/^[a-z-_0-9]*$/",$uname)) 
		{
			if(strlen($uname) >= 2)
			{
				require_once '../Model/connectionDb.php';
				$conn = db_conn();
				$selectQuery = "SELECT * FROM `deliveryman` WHERE uname = :uname";
				try
			    {
			        $stmt = $conn->prepare($selectQuery);
	...
	{  
		require_once '../Model/modeldeliveryman.php';
